...
export function EvidenceSequence() {
    ...
    return (
        ...
        >
            ...
            >
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#d2d7cf] pb-5 font-mono text-xs uppercase tracking-widest text-[#65736f]">
                ...
                <div className="mt-5 flex flex-wrap items-end justify-between gap-4">
                    <div>
                        ...
                        <p className="mt-1 font-mono text-xs uppercase tracking-widest text-[#74807d]">
                            {activeSample.citations} provider citation{activeSample.citations === 1 ? "" : "s"} · illustrative
                        </p>
                    </div>
                    <span className="font-mono text-xs uppercase tracking-widest text-[#74807d]">{activeSample.reference}</span>
                </div>
            </article>
                ...
                <div className="flex flex-wrap" role="group" aria-label="Choose an illustrative answer sample">
                    {SAMPLES.map((sample, index) => (
                        <button
                            ...
                            className="aelo-sample-button relative inline-flex min-h-11 items-center gap-2 px-3 text-left font-mono text-xs uppercase tracking-widest text-[#65736f] transition-colors duration-200 hover:text-[#1d2523] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#416a88] focus-visible:ring-offset-2"
                        >
                ...
                {ENGINE_ROWS.map((engine, index) => {
                    ...
                    return (
                        <div key={engine.name} className="grid gap-3 border-b border-[#cbd2cb] px-4 py-5 last:border-b-0 sm:grid-cols-[36px_96px_1fr_110px] sm:items-center">
                            <span className="font-mono text-xs text-[#7a8783]">0{index + 1}</span>
                            ...
                            <span className="font-mono text-xs text-[#65736f] sm:text-right">{engine.mentions}/{engine.samples} · 95% {engine.range}</span>
                        </div>
                    );
                })}
            </div>
            <p className="mt-4 text-xs leading-relaxed text-[#65736f]">Illustrative structure, not a customer result. A live receipt only shows answers and evidence returned by the provider.</p>
        </div>
    );
}
