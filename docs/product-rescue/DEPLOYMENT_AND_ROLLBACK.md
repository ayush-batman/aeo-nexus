...
2. Apply migrations in numeric order: 025, 026, 027, 028, 029, 030, 031, then 032. Migration 024 must already exist in the target because API-key routes depend on it.
3. Verify migration objects and constraints before deploying code. Do not deploy the rescued code against a schema below 032.
...
If schema rollback is required, export affected rows first and reverse from 032 downward:
...
Migrations 025–032 were not applied locally or to production in this task. The exact SQL must be rehearsed on a disposable or staging Supabase database before approval.

## Local verification result

- `npm run lint`: passed with 0 errors; 67 warnings remain.
- `npm run typecheck`: passed.
- `npm run typecheck:mcp`: passed.
- `npm test`: 85 passed.
- `npm run build -- --webpack`: passed after a clean `.next`; 136 pages/routes generated.
- Public/auth browser checks: passed at four target widths with no console errors.
...
1. Create an isolated staging Supabase project, apply 025–032 in order, and run the role/tenant/migration checks with an authorized test owner plus viewer.
