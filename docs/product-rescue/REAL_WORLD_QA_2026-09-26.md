...
### Local follow-up, 26 September

The current branch now separates mention-based visibility from an AI-classified recommendation status, with a short quote that must appear in the saved answer. This is an additive, **local-only** change; historical receipts remain unassessed and the live product has not received it. The Buffer alternatives receipt above was reopened against the non-production test backend through the local app: it now says “Recommendation not assessed” rather than implying that a mention is an endorsement. Desktop and 390px mobile layouts loaded without a page overlay, console error, failed network request, or horizontal overflow. A new classified receipt has not been browser-tested because this backend code has not been deployed. The classifier is still an AI interpretation, not a verified ground-truth label; the user must be able to inspect the quote and full answer.

A subsequent local change makes the free scan explicit that its competitor list was never configured. It derives up to eight possible names from strongly formatted entries in the saved raw answer, with verbatim excerpts, and labels them unverified. It does not claim automatic verified competitor discovery or that an empty candidate list means no rivals appeared. This is not yet deployed.

The homepage free-scan widget now opts into an immediate queued receipt after durable reservation; the existing receipt polls for saved evidence. Other API callers retain the previous wait-and-return path. This removes the provider-generation wait from the homepage request, but does not shorten Gemini's generation time. No new paid scan was started solely to benchmark this local change.
...
| P2 | `convex/publicScanActions.ts`, public receipt | Public scan passes an empty competitor list, so named rivals are persisted as `[]`. | Local fix exposes `not_configured` and up to eight answer-backed possible names without claiming verified rivals. The saved Buffer alternatives answer yields Hootsuite, Sendible, Sprout Social, Planable, SocialPilot, and Agorapulse, all with verbatim excerpts. Not deployed; verified competitor tracking remains open. |
| P2 | public receipt rendering | First local Convex timeout caused full-page error; no receipt-level retry UI. | Local route-level retry screen added without exposing raw errors or claiming the scan failed. A forced local connection refusal rendered it at desktop and mobile widths; retry stayed on the same receipt link. Not deployed. The deliberately failed request produced expected development error logs, plus an unrelated script-render warning on that failure path. |
